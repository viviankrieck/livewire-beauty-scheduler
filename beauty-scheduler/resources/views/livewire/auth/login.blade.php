<div class="flex flex-col gap-6">
    <x-auth-header :title="__('Beauty Scheduler')" :description="__('Digite seu e-mail e senha abaixo para fazer login')" />
    ...
    <form method="POST" wire:submit="login" class="flex flex-col gap-6">
        <!-- Email Address -->
        <flux:input wire:model="email" :label="__('Digiite seu email')" type="email" required autofocus
            autocomplete="email" placeholder="email@example.com" />

        <!-- Password -->
        <div class="relative">
            <flux:input wire:model="password" :label="__('Digite sua senha')" type="password" required
